...
for index, character in enumerate(unique_characters, start=1):
    ttk.Label(tts_frame, text=character).grid(row=index, column=0, padx=5, pady=5)
    combobox = ttk.Combobox(tts_frame)
    combobox.grid(row=index, column=1, padx=5, pady=5)
    speaker_dropdown_mapping[character] = combobox
    preview_button = ttk.Button(tts_frame, text="Preview", command=lambda char=character: preview_voice(char))
