...
from tkinter import ttk, messagebox, Toplevel, Listbox, filedialog
import pandas as pd

def generate_csv(quotes_file, tokens_file, entities_file):
    # Load the files
...
def launch_gui():
    ...
    for speaker_info in quotes_df['Speaker'].unique():
        char_id, names = speaker_info.split(':', 1)
...
def show_file_selector():
    ...
    entities_file = filedialog.askopenfilename(title="Select the entities file")
    if not entities_file:
        return

    generate_csv(quotes_file, tokens_file, entities_file)
    launch_gui()

# Start with the file selector
show_file_selector()
