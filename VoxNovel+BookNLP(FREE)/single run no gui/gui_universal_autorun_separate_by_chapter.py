...
root = tk.Tk()
root.title("Tortoise TTS GUI")
root.geometry("1200x800")

# Frame for Character Voice Dropdowns
voice_selection_frame = ttk.LabelFrame(root, text="Character Voices")
voice_selection_frame.pack(fill="x", expand="yes", padx=10, pady=10)
...
# Create a dropdown for each speaker
for speaker in data['Speaker'].unique():
    speaker_label = ttk.Label(voice_selection_frame, text=speaker)
    speaker_label.pack(side="top", fill="x", expand="yes")

    voice_combobox = ttk.Combobox(voice_selection_frame, values=voice_actors, state="readonly")
...
def update_progress(index, total):
    ...
    progress_label.config(text=f"{progress:.2f}% done")
...
voice_selection_frame = ttk.Frame(root)
voice_selection_frame.pack(pady=10, fill=tk.X)
...
def load_book():
    ...
    # Load the text from the CSV and insert into the text display
    for index, row in data.iterrows():
        speaker = row['Speaker']
        text = row['Text']
        text_display.insert(tk.END, f"{speaker}: {text}\n\n")
